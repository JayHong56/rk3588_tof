...
ADIController::ADIController(
    ...
    : m_cameraInUse(-1), m_frameRequested(false),
      m_recorder(new ADIToFRecorder()), m_networkOnly(false),
      m_networkStream(new ADINetworkToFStream()) {
    ...
    if (m_cameras.size()) {
        ...
        LOG(INFO) << "No local cameras attached to this controller.";
    }
}

ADIController::~ADIController() {
    StopCapture();
    if (!m_networkOnly && m_cameraInUse != -1) {
        m_cameras[static_cast<unsigned int>(m_cameraInUse)]->stop();
    }
}

void ADIController::configureNetworkOnly(
    const ADINetworkToFStream::Config &config) {
    if (m_networkOnly && m_networkStream) {
        // Keep an already-open Machine A connection alive when only mode/TOFI
        // parameters are being updated before Play. Full teardown is handled
        // by ADINetworkToFStream::~ADINetworkToFStream().
        if (!m_networkStream->isRunning()) {
            m_networkStream->stop();
        }
    } else {
        StopCapture();
    }
    ...
    m_cameras.clear();
    if (!m_networkStream) {
        m_networkStream.reset(new ADINetworkToFStream());
    }
    m_networkStream->configure(config);
}

bool ADIController::isNetworkOnly() const { return m_networkOnly; }

std::string ADIController::getNetworkStatusText() const {
    if (!m_networkStream) {
        return "Network listener is not configured";
    }
    return m_networkStream->statusText();
}

void ADIController::ConnectNetwork() {
    if (m_networkOnly && m_networkStream) {
        m_networkStream->start();
    }
}
...
void ADIController::StopCapture() {
    if (m_networkOnly) {
        if (m_networkStream) {
            m_networkStream->stopRemoteCapture();
        }
        m_queue.erase();
        return;
    }
...
void ADIController::setMode(const std::string &mode) {
    if (m_networkOnly) {
        if (m_networkStream) {
            m_networkStream->setFrameType(mode);
        }
        return;
    }
...
std::shared_ptr<aditof::Frame> ADIController::getFrame() {
    ...
    if (m_networkOnly) {
        return m_networkStream->getFrame();
    }
...
void ADIController::requestFrame() {
    ...
    } else if (m_networkOnly) {
        m_networkStream->requestFrame();
    } else {
        std::unique_lock<std::mutex> lock(m_requestMutex);
...
bool ADIController::hasCamera() const { return m_networkOnly || !m_cameras.empty(); }
