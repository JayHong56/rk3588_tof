...
std::string getJsonString(cJSON *json, const char *key,
...
uint16_t getJsonU16(cJSON *json, const char *key, uint16_t fallback) {
...
uint32_t getJsonU32(cJSON *json, const char *key, uint32_t fallback) {
    std::string value = getJsonString(json, key, "");
    if (value.empty()) {
        return fallback;
    }
...
ADIMainWindow::ADIMainWindow() : m_skipNetworkCameras(true) {
    ...
    if (config_json != NULL) {
        ...
        // Legacy setting kept only for compatibility; this viewer no longer
        // opens SDK network cameras.
        ...
        m_networkListenIp = getJsonString(config_json, "net_listen_ip",
                                          m_networkListenIp);
        m_networkListenPort = getJsonU16(config_json, "net_listen_port",
                                         m_networkListenPort);
        m_networkCcbFile = getJsonString(config_json, "net_tofi_ccb",
                                         m_networkCcbFile);
        m_networkCfgFile = getJsonString(config_json, "net_tofi_cfg",
                                         m_networkCfgFile);
        m_networkReceivedCcbFile = getJsonString(config_json, "net_received_ccb",
                                                 m_networkReceivedCcbFile);
        m_networkReceivedCfgFile = getJsonString(config_json, "net_received_cfg",
                                                 m_networkReceivedCfgFile);
        m_networkIniFiles = getJsonString(config_json, "DEPTH_INI",
                                          m_networkIniFiles);
        m_networkDefaultMode = getJsonU16(config_json, "net_default_mode",
...
void ADIMainWindow::RefreshDevices() {
    ...
    // Search for configuration files with .json extension. These files still
    // provide GUI ranges, mode names and DEPTH_INI/CCB inputs for TOFI.
    configSelection = -1;
...
bool ADIMainWindow::probeNetworkCollector(uint32_t timeoutMs) {
    const std::string listenIp =
        m_networkListenIp.empty() ? std::string("0.0.0.0") : m_networkListenIp;

#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32)
    (void)timeoutMs;
    m_networkProbeStatus =
        "Network probe is implemented for Linux builds; virtual device is always shown";
    return true;
#else
    try {
        tof_net::Socket probeSocket;
        probeSocket.bind_listen(listenIp, m_networkListenPort, 1);

        fd_set readfds;
        FD_ZERO(&readfds);
        FD_SET(probeSocket.fd(), &readfds);

        timeval timeout;
        timeout.tv_sec = static_cast<long>(timeoutMs / 1000);
        timeout.tv_usec = static_cast<long>((timeoutMs % 1000) * 1000);

        const int rc = ::select(probeSocket.fd() + 1, &readfds, nullptr, nullptr,
                                &timeout);
        if (rc > 0 && FD_ISSET(probeSocket.fd(), &readfds)) {
            std::string peerIp;
            uint16_t peerPort = 0;
            tof_net::Socket client = probeSocket.accept_one(&peerIp, &peerPort);
            client.close();
            probeSocket.close();

            std::ostringstream ss;
            ss << "Detected Machine A tof_net_collect from " << peerIp << ":"
               << peerPort << "; click Open Device to accept the real connection";
            m_networkProbeStatus = ss.str();
            my_log.AddLog("%s\n", m_networkProbeStatus.c_str());
            return true;
        }

        if (rc == 0) {
            std::ostringstream ss;
            ss << "No tof_net_collect connection detected on " << listenIp << ":"
               << m_networkListenPort << " within " << timeoutMs << " ms";
            m_networkProbeStatus = ss.str();
            return false;
        }

        std::ostringstream ss;
        ss << "Network probe select() failed: " << std::strerror(errno);
        m_networkProbeStatus = ss.str();
        return false;
    } catch (const std::exception &e) {
        std::ostringstream ss;
        ss << "Network probe failed on " << listenIp << ":" << m_networkListenPort
           << ": " << e.what();
        m_networkProbeStatus = ss.str();
        return false;
    }
...
void ADIMainWindow::showOpenDeviceWindow() {
    ...
    if (ImGui::TreeNode("Open Device")) {
        ...
        ImGui::Separator();
        ImGui::TextWrapped("Network endpoint on Machine B: %s:%u",
                           m_networkListenIp.empty() ? "0.0.0.0"
                                                     : m_networkListenIp.c_str(),
                           m_networkListenPort);
        ImGui::TextWrapped("Refresh probe: %s", m_networkProbeStatus.c_str());
        if (view && view->m_ctrl && view->m_ctrl->isNetworkOnly()) {
            const std::string netStatus = view->m_ctrl->getNetworkStatusText();
            ImGui::TextWrapped("Network status: %s", netStatus.c_str());
        } else {
            ImGui::TextWrapped("Network status: not connected. Start Machine A tof_net_collect, click Refresh Devices, then Open Device.");
        }
        //ImGui::SameLine();
#if defined(Debug) && defined(_WIN32)
...
void ADIMainWindow::InitCamera() {
    ...
    std::string version = aditof::getApiVersion();
    LOG(INFO) << "Preparing network RAW ToF listener.\n";
    m_controller = std::make_shared<adicontroller::ADIController>(
        std::vector<std::shared_ptr<aditof::Camera>>());

    view = std::make_shared<adiviewer::ADIView>(m_controller,
                                                "ToF Network Viewer " + version);
    ...
    if (m_configFiles.empty() || configSelection < 0 ||
        configSelection >= static_cast<int>(m_configFiles.size())) {
        _usesExternalModeDefinition = false;
        _cameraModes = {"lr-qnative", "sr-qnative", "lr-native", "sr-native",
                        "lr-mixed", "sr-mixed", "pcm-native"};
    } else {
    ...
    if (view && view->m_ctrl) {
        view->m_ctrl->ConnectNetwork();
    }

    my_log.AddLog("Network RAW endpoint opened. Waiting for Machine A reconnect; Play sends StartCapture.\n");
    cameraWorkerDone = true;
}

void ADIMainWindow::prepareCamera(std::string mode) {
    if (mode.empty()) {
        my_log.AddLog("Error: Invalid network frame mode!\n");
        return;
    }

    configureNetworkRawInput(mode);
    my_log.AddLog("Network RAW listener configured for mode %s.\n",
                  mode.c_str());
    cameraWorkerDone = true;
}

std::string ADIMainWindow::selectNetworkIniForMode(
    const std::string &mode) const {
    std::vector<std::string> iniFiles = splitSemicolonList(m_networkIniFiles);

    auto containsMode = [&mode](const std::string &path) {
        return path.find(mode) != std::string::npos;
    };

    auto fileReadable = [](const std::string &path) {
        if (path.empty()) {
            return false;
        }
    ...
    if (!iniFiles.empty()) {
        // Prefer exact mode match, but only if the file is actually readable.
        auto it = std::find_if(iniFiles.begin(), iniFiles.end(), containsMode);
        if (it != iniFiles.end() && fileReadable(*it)) {
            return *it;
        }
        // Try any readable entry from the DEPTH_INI list.
        auto rd = std::find_if(iniFiles.begin(), iniFiles.end(), fileReadable);
        if (rd != iniFiles.end()) {
            return *rd;
        }
    }

    // Fallback: search common paths on disk for the matching INI file.
    // (Files like RawToDepthAdsd3500_sr-mixed.ini live in
    //  ../tof-net-collect/config/, not ./config/.)
    std::vector<std::string> candidates;
    if (!mode.empty()) {
        candidates.push_back("./config/RawToDepthAdsd3500_" + mode + ".ini");
        candidates.push_back("./config/RawToDepthAdsd_" + mode + ".ini");
    ...
    auto readable = std::find_if(candidates.begin(), candidates.end(), fileReadable);
    if (readable != candidates.end()) {
        return *readable;
    }
...
void ADIMainWindow::configureNetworkRawInput(const std::string &mode) {
    if (!view || !view->m_ctrl) {
        return;
    }

    adicontroller::ADINetworkToFStream::Config config;
    config.listenIp = m_networkListenIp.empty() ? "0.0.0.0" : m_networkListenIp;
    config.listenPort = m_networkListenPort;
    config.receivedCcbFile = m_networkReceivedCcbFile.empty()
                                 ? std::string("./config/received_machine_a_module.ccb")
                                 : m_networkReceivedCcbFile;
    config.receivedCfgFile = m_networkReceivedCfgFile.empty()
                                 ? std::string("./config/received_machine_a_module.cfg")
                                 : m_networkReceivedCfgFile;
    {
        std::ifstream received(config.receivedCcbFile, std::ios::binary);
        config.ccbFile = received.good() ? config.receivedCcbFile : m_networkCcbFile;
    }
    {
        std::ifstream received(config.receivedCfgFile, std::ios::binary);
        if (received.good()) {
            config.cfgFile = config.receivedCfgFile;
        } else {
            // Only use the local fallback CFG if it actually exists on disk.
            // ADSD3500 devices do not have CFG in EEPROM, so an empty cfgFile
            // is the correct default.
            std::ifstream localCfg(m_networkCfgFile, std::ios::binary);
            config.cfgFile = localCfg.good() ? m_networkCfgFile : std::string();
        }
    }
    config.iniFile = selectNetworkIniForMode(mode);
    config.frameType = mode;
    config.mode = m_networkDefaultMode;
    ...
    my_log.AddLog("Listening on %s:%u, TOFI INI=%s, CCB=%s, CFG=%s\n",
...
void ADIMainWindow::displayInfoWindow(ImGuiWindowFlags overlayFlags) {
    ...
    if (ImGui::Begin("Information Window", nullptr,
        ...
        if (view->m_ctrl->isNetworkOnly()) {
            ImGui::TextWrapped("Network: %s",
                               view->m_ctrl->getNetworkStatusText().c_str());
        }
    }
    ImGui::End();
...
bool ADIMainWindow::checkCameraSetToReceiveContent(
    ...
    if (view && view->m_ctrl && view->m_ctrl->isNetworkOnly()) {
        return contentType == "depth" || contentType == "ir" ||
               contentType == "xyz";
    }
