...
class ADIController {
  public:
    ...
    /**
		* @brief Configure controller to receive RAW ToF frames from the network.
		*/
    void configureNetworkOnly(const ADINetworkToFStream::Config &config);

    /**
		* @brief Returns true when the controller is in network RAW listener mode.
		*/
    bool isNetworkOnly() const;

    /**
		* @brief Human-readable status of the network listener / TOFI pipeline.
		*/
    std::string getNetworkStatusText() const;
  ...
  private:
    ...
    bool m_networkOnly;
    std::unique_ptr<ADINetworkToFStream> m_networkStream;
};
} // namespace adicontroller
#endif
