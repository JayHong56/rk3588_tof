...
std::shared_ptr<aditof::Frame> ADIController::getFrame() {
    ...
    // Non-blocking: returns nullptr immediately if no frame is
    // available. The synchronize function will call requestFrame()
    // and return, keeping the GUI responsive even when the camera
    // is slow or unreachable.
    std::shared_ptr<aditof::Frame> frame;
    m_queue.try_dequeue(frame);
    return frame;
}
...
void ADIController::captureFrames() {
    while (!m_stopFlag.load()) {
        ...
        // Consume this request before network I/O. A new request arriving while
        // I/O is in progress remains set and is handled by the next iteration.
        ...
        if (status != aditof::Status::OK) {
            ...
            // Do NOT enqueue empty/corrupt frames. The blocking dequeue
            // in getFrame() will wait until a valid frame arrives.
            panicCount++;
            LOG(WARNING) << "requestFrame failed (panicCount=" << panicCount
                         << "), dropping frame";
            continue;
        }
