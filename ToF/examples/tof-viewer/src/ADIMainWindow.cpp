...
bool ADIMainWindow::prepareCamera(std::string mode) {
    aditof::Status status = aditof::Status::OK;
    ...
    if (status != aditof::Status::OK) {
        ...
        my_log.AddLog("Could not start camera. Restart the camera service or power-cycle the camera.\n");
        return false;
    }
...
void ADIMainWindow::PlayCCD(int modeSelect, int viewSelect) {
    ...
    if (view->m_ctrl->hasCamera()) {
        ...
        if (modeSelectChanged != modeSelect || captureSeparateEnabled ||
            ...
            if (!prepareCamera(m_cameraModes[modeSelection].second)) {
                isPlaying = false;
