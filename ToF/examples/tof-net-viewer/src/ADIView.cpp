...
void clearFloatingPointStateForDisplay() {
    // Some TOFI/libm builds can leave floating-point exception flags set.
    // The viewer display code must never terminate on flat images where
    // max == min; clear the flags before normalization.
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (showIRWindow) {
            ...
            if (beginDisplayDepthImage) {
                if (!m_ctrl->isNetworkOnly() && m_ctrl->hasCamera()) {
                    aditof::Status status;

                    // Initialize first TOF camera. This branch is retained for
                    // the legacy demo render path only; the network viewer
                    // path never opens a local camera.
                    ...
                    ImGui::Image((void *)(intptr_t)image_texture,
                                 sourceImageDimensions);
                } else if (m_ctrl->isNetworkOnly()) {
                    ImGui::Text("Depth is provided by the network RAW listener.");
                }
            }
            ImGui::End();
        }
...
void ADIView::_displayIrImage() {
    while (!m_stopWorkersFlag) {
        ...
        if (frameHeight <= 0 || frameWidth <= 0 || ir_video_data == nullptr) {
            continue;
        }

        size_t imageSize = static_cast<size_t>(frameHeight) *
                           static_cast<size_t>(frameWidth);
        size_t bgrSize = 0;
        ir_video_data_8bit = new uint8_t[imageSize * 3];
        ...
        clearFloatingPointStateForDisplay();
...
void ADIView::_displayDepthImage() {
    while (!m_stopWorkersFlag) {
        ...
        if (frameHeight <= 0 || frameWidth <= 0 || depth_video_data == nullptr) {
            continue;
        }
...
void ADIView::_displayPointCloudImage() {
    while (!m_stopWorkersFlag) {
        ...
        if (frameHeight <= 0 || frameWidth <= 0 || pointCloud_video_data == nullptr) {
            continue;
        }
