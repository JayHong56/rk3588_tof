...
class ADIMainWindow {
  ...
  private:
    ...
    /**
		* @brief Configure network-only RAW listener and TOFI inputs.
		*/
    void configureNetworkRawInput(const std::string &mode);

    /**
		* @brief Select a TOFI ini file from DEPTH_INI for the requested mode.
		*/
    std::string selectNetworkIniForMode(const std::string &mode) const;

    /**
     * @brief Temporarily listen on the configured network port to detect
     *        whether Machine A's tof_net_collect is running and reconnecting.
     */
    bool probeNetworkCollector(uint32_t timeoutMs);
    ...
    std::string m_networkListenIp = "0.0.0.0";
    uint16_t m_networkListenPort = 5000;
    std::string m_networkCcbFile;
    std::string m_networkCfgFile;
    std::string m_networkReceivedCcbFile = "./config/received_machine_a_module.ccb";
    std::string m_networkReceivedCfgFile = "./config/received_machine_a_module.cfg";
    std::string m_networkIniFiles;
