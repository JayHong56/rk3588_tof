...
## Live CCB from Machine A

`tof-net-viewer` accepts a `CcbFile` protocol message from Machine A before streaming starts. The received CCB is saved to `net_received_ccb` from `tof-viewer_config.json` (default: `./config/received_machine_a_module.ccb`) and TOFI is reinitialized with that file. `net_tofi_ccb` is now only a fallback until the live module CCB arrives.

Expected log on Machine B:

```text
Received current module CCB from Machine A: <bytes> bytes, source=<path>, saved=./config/received_machine_a_module.ccb; TOFI will use received CCB
TOFI initialized: mode=0, frame_type=sr-native, ini=..., ccb=./config/received_machine_a_module.ccb
```

## v14 network calibration transfer

`tof-net-viewer` now accepts both calibration files from Machine A before streaming starts:

- `MessageType::CcbFile` -> saved to `net_received_ccb` (default `./config/received_machine_a_module.ccb`)
- `MessageType::CfgFile` -> saved to `net_received_cfg` (default `./config/received_machine_a_module.cfg`)

TOFI initialization uses `INI + received CCB + received CFG`. If the received files are not present yet, the viewer falls back to `net_tofi_ccb` and `net_tofi_cfg` from `tof-viewer_config.json`.

Expected Machine B log sequence:

```text
Received current module CCB from Machine A: ... bytes, saved=./config/received_machine_a_module.ccb
Received current module CFG from Machine A: ... bytes, saved=./config/received_machine_a_module.cfg
TOFI init input: mode=..., frame_type=..., ini=..., ccb=..., cfg=...
TOFI initialized: mode=..., frame_type=..., ini=..., ccb=..., cfg=...
```
